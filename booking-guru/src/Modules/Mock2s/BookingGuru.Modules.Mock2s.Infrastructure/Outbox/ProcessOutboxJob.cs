...
{
    private const string ModuleName = "Mock2s";
        ...
        IDbTransaction transaction)
    {
        string sql =
            $"""
             ...
             FROM {Schemas.Mock2s}.{nameof(OutboxMessage)}
             WHERE processedOnUtc IS NULL
             ORDER BY occurredOnUtc
             OFFSET 0 ROWS
             FETCH NEXT {outboxOptions.Value.BatchSize} ROWS ONLY 
             """;
        ...
        Exception? exception)
    {
        const string sql =
            $"""
            UPDATE {Schemas.Mock2s}.{nameof(OutboxMessage)}
